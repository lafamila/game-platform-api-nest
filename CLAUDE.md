...
- **Search engines** (pure modules, no NestJS deps so a worker can load them cheaply): `src/games/othello-ai.ts` (`searchOthelloMove`) — iterative-deepening negamax + alpha-beta + Zobrist TT, 3-phase eval (mobility/frontier/stability + corner and X/C-square terms), exact endgame solve at ≤14 empties. `src/games/gomoku-ai.ts` (`searchGomokuMove`) uses an `Int8Array` position with affected-line incremental evaluation, dual-`Uint32` Zobrist TT with verifier keys and legal-move validation, lazy legal candidate quota (≤16/node), forced-response depth extension, VCF, and bounded VCT/refutation search. Pattern semantics distinguish open/closed/broken threats, deduplicate winning points, and never score a white overline as exact five. Both return the best move from the last **completed** depth.
- **Time management**: `GOMOKU_AI_BUDGET_MS` is a late-game maximum. Gomoku uses phase caps (0–4 stones: 3s, 5–8: 8s, later: configured maximum) and does not start a new depth when the measured growth predicts it cannot finish. The optional search node cap is for deterministic regression/benchmark tests only; production remains time-budgeted.
- **Shared util**: `src/games/engine/zobrist.ts` retains splitmix64/BigInt keys for Othello and provides deterministic dual 32-bit keys for Gomoku. Protocol and diagnostics types are in `src/games/engine/ai-worker-protocol.ts`.
- **Worker model**: `src/games/engine/ai-worker.ts` runs hard search off the main event loop. `src/games/engine/ai-worker-pool.ts` caps concurrency (`GAME_AI_WORKER_POOL_SIZE`, default 2) with FIFO waiting; the request's absolute deadline begins at submission, so queue time reduces the remaining think time instead of adding another full budget. A deadline kill returns the last completed interim move. Spawn/serialization failure still falls back to the sync engine. The service routes `hard` to the worker only when the budget ≥400ms; smaller test budgets run synchronously.
- **Decision telemetry and loss corpus**: hard Gomoku writes best-effort rows to `ai_decisions` before applying the AI move: session/ply, board hash, engine version, move, phase budget/elapsed time, completed depth, search/VCF/VCT/evaluation/forbidden/candidate counts, score, PV, and exit reason. Telemetry failure is logged but never aborts a game. With `DATABASE_URL` set, `npm run corpus:gomoku-hard-losses -- [output.json]` extracts every recorded AI decision from completed hard local-AI losses, reconstructing the board before that ply from `moveHistory` for regression fixtures.
