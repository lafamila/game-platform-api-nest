...
- **Shared util**: `src/games/engine/zobrist.ts` provides deterministic dual 32-bit keys; the primary half indexes each TT and the verifier half rejects collisions. Protocol and diagnostics types are in `src/games/engine/ai-worker-protocol.ts`. `npm run benchmark:othello-hard -- 10000` reports opening/midgame/endgame depth, nodes, NPS, elapsed time, and exit reason without gating CI.
